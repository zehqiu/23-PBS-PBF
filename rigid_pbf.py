    ...
    def PBF_solver(self):
        ...
        for p_i in self.ps.positions:
            ...
            for j in range(self.ps.particle_num_neighbors[p_i]):
                ...
                if p_j < 0:
                    break
                if p_j < self.ps.n_fluid_particles :
                    ...
                    pos_ji = pos_i - self.ps.positions[p_j]
                    if pos_ji.norm() < particle_diameter :
                        dist = self.ps.positions[p_j] - pos_i
                        self.ps.positions[p_i] = self.ps.old_positions[p_i]
                        self.ps.velocities[p_i] += dist * time_delta
                    pos_ji = pos_i - self.ps.positions[p_j]
                    grad_j = spiky_gradient(pos_ji, h)
                    grad_i += grad_j
                    sum_gradient_sqr += grad_j.dot(grad_j)
